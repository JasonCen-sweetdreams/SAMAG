...
parser.add_argument('--diameter_mode', 
                    type=str, 
                    default="both", 
                    choices=['approx', 'both'],
                    help='Diameter calculation mode: "approx" for effective diameter, "both" for both effective and exact diameters.')

parser.add_argument('--num_snapshots',
                    type=int,
                    default=30,
                    help='The number of snapshots to evaluate.')
...
def visualize_sn_graphs(graph_generator,
    ...
    for DG, G, date_str, action_graph, positions in graph_generator:
        follower_diameter = safe_diameter(DG)
        # friend_diameter = safe_diameter(G)
...
def calculate_social_matrix(graph_lists:list,
    ...
    for DG, G, action_graph, date_str, positions in tqdm(graph_lists):
        ...
        #     continue
        for graph_type in graph_types:
            print(f"Evaluating for {graph_name} {graph_type}...")
            ...
            df = pd.DataFrame(matrix, index=[graph_name])
            print(df)
            
            graph_list_map[graph_type].append(df)
            print(f"finished for {graph_name} {graph_type}")
...
def plot_power_law_distribution(graph: nx.DiGraph, title: str, save_path: str = None):
    ...
    ax.text(0.65, 0.95, text_str, transform=ax.transAxes, fontsize=14,
...
if __name__ == "__main__":
    ...
    # visualize_social(df_root=save_root, save_root=vis_save_root)
    # print("finish plot social network matrix!")

    # if graph_lists:
    #     plot_single_graph(data_root=data_root, csv_file_index=-1, save_root=vis_save_root)
    # print("finish plot graph!")
