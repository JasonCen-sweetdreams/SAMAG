...
from Emulate.tool.movie_html import AsyncMovieHtmlLoader
...
def _get_movie_relevant_documents(
    ...
    try:
        ...
        if k_final:
            ...
            try:
                k = retriever.search_kwargs["k_final"]
                print(f"Using K_final as k")
            except:
                k = retriever.search_kwargs["k"]
                print(f"Using default k")
        ...
        for query in query_list:
            ...
            docs = retriever.get_relevant_documents(query, user_id=user_id)
...
def _get_reviews_for_movie(movie_id: int, n_latest: int = 5, manager: Any = None) -> str:
    """
    实际执行获取评论的逻辑函数。
    注意：我们将 MovieManager 实例作为参数传入，以便访问其数据。
    """
    try:
        result_dict = manager.get_reviews_for_movie(movie_id=movie_id, n_latest=n_latest)
        # 将结果格式化为对LLM更友好的字符串
        return ServiceResponse(status=ServiceExecStatus.SUCCESS, content=result_dict.get("reviews", "No reviews found."))
    except Exception as e:
        return ServiceResponse(status=ServiceExecStatus.ERROR, content=str(e))

def create_get_reviews_tool(
    manager: Any, # 传入 MovieManager 实例
    name: str,
    description: str,
) :
    """
    创建用于获取电影评论的工具。
    """
    # 使用 GraphServiceFactory 进行标准封装
    return GraphServiceFactory.get(
        _get_reviews_for_movie,
        name=name,
        description=description,
        manager=manager
    )