...
from Emulate.utils.graph import build_article_graph
from Emulate.retriever import retriever_registry

from Emulate.output_parser import find_and_load_json
...
class ArticleManager(BaseManager):
    ...
    def load_data(cls,
        ...
        logger.info(f"Meta Info Read Done!")
        ...
        if os.path.exists(log_info_path):
            ...
            article_meta_data, author_data = pre_process(article_meta_data, 
                                                         ...
                                                         cur_time,
                                                         load_history= False)
        logger.info(f"Current article_written_num: {article_written_num}")
    ...
    def update_db(self):
        ...
        pyg_graph_data, node_id_to_title, title_to_node_id, index_to_doc = \
            build_article_graph(self.article_meta_data, self.author_data, docs)
        ### Raw RAG
        if  self.retriever_kwargs.get("type", "graph_vector_retriever") == "graph_vector_retriever":
        ...
        ### with Graph
            ...
            self.graph_retriever = retriever_registry.from_db(
                # type="community_graph_retriever_for_article",     # 社区检索
                type = self.retriever_kwargs.get("Graph_RAG_type", "graph_structure_retriever_for_article"),
                ...
                allow_co_citation=self.retriever_kwargs.get("allow_co_citation", False),
                allow_co_author=self.retriever_kwargs.get("allow_co_author", False)
            )
            self.retriever_kwargs.update({
                "type":                  self.retriever_kwargs.get("type", "hybrid_graph_vector_retriever_for_article"),
                "vectorstore":           self.db,
                "compare_function_type": "article",
                "search_type":           self.retriever_kwargs.get("search_type", "similarity"),
                "search_kwargs":         self.retriever_kwargs.get("search_kwargs", {"k":5}),
                "graph_retriever":       self.graph_retriever,
                "graph_hops":            self.retriever_kwargs.get("graph_hops", 1),
                "expansion_strategy":    self.retriever_kwargs.get("expansion_strategy", "simple")
            })
            # self.retriever_kwargs["vectorstore"] = self.db
            # self.retriever_kwargs["compare_function_type"] = "article"
            self.retriever = retriever_registry.from_db(**self.retriever_kwargs)
        
        self.semantic_retriever = retriever_registry.from_db(
            type="graph_vector_retriever",
            vectorstore=self.db,
            compare_function_type="article",
            search_type="similarity",
            search_kwargs={"k":5, "score_cite": True}
        )
...
"""
        ...
        add_indexs = []
        from Emulate.output_parser.base_parser import find_and_load_json
        try:
