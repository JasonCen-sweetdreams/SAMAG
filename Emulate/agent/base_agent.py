...
from Emulate.output_parser.base_parser import AgentOutputParser
import json
from Emulate.prompt import MODEL
from Emulate.utils.count import count_prompt_len, parse_prompt
    ...
    def step(self,
        ...
        for i in range(self.max_retrys):
            ...
            if isinstance(res_json, str):
                # --- NEW: 增加日志记录，输出有问题的原始字符串 ---
                logger.warning(
                    f"Agent [{self.name}] received a string instead of a dict from LLM. "
                    f"Attempting to parse JSON from it. Raw string content: '{res_json}'"
                )
                # --- 日志记录结束 ---
                ...
                try:
                    ...
                    if json_match:
                        ...
                        res_json = {"fail": True, "reason": "No JSON block found in string response."}
                except json.JSONDecodeError:
                    res_json = {"fail": True, "reason": f"Failed to decode JSON from string: {res_json}"}
        ...
        return Msg("user",content= f"FAIL TO PARSE, Error in step process, Prompt: {prompt}",role = "user", fail = True)
        ...
        if  message.content == "reply_prompt":
            ...
            try:
                ...
                if "fail" in output.keys():
                    return Msg("user",content= f"FAIL TO PARSE, output: {output}",role = "user",fail = True)
                ...
                print(e)
                return Msg("user",content=f"FAIL TO PARSE, Exception: {e}",role = "user",fail =True)
            
        return Msg("user",content=f"FAIL TO PARSE, message: {message.content}",role = "user",fail =True)
