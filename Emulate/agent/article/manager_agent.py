from Emulate.agent.tool_agent import ToolAgent
from Emulate.manager import ArticleManager
from agentscope.message import Msg
from langchain_core.prompts import PromptTemplate
from Emulate.tool import (create_article_retriever_tool,
                           create_article_online_retriever_tool)
from typing import List,Dict,Union,Any
from datetime import datetime

from Emulate.agent.base_manager import ManagerAgent
import copy
...
Your paper should cite at least {min_citations_online} papers!".format_map(article_write_configs),
    ...
    def update(self,
        ...
        if update_retriever:
            grag_retriever = self.article_manager.get_retriever()
            semantic_retriever = self.article_manager.get_semantic_retriever()
            emb_model = self.article_manager.embeddings
            
            # proposal检索器
            retriever_tool_func_p,retriever_tool_dict_p = create_article_retriever_tool(
                semantic_retriever,
                "retrieve_for_proposal",
        "Search for relevant papers, so as to **propose/refine a academic proposal or make suggestion about the current proposal**. \
    Several papers should help you to propose a proposal or refine your current proposal.",
                #     These papers should be included in your paper's citations if you use them in your paper. \
            ...
            # 全文检索器
            hybrid_alpha = self.article_manager.retriever_kwargs.get("hybrid_alpha", 0.5)
            retriever_tool_func,retriever_tool_dict = create_article_retriever_tool(
                grag_retriever,
                "retrieve_for_final_paper",
        "Search for relevant papers, so as to generate your final paper based on your final proposal. \
    These papers should be included in your paper's citations if you use them in your paper. \
    Your proposal should cite at least {min_citations_db} papers!".format_map(self.article_write_configs),
            ...
            tools = {
            "retrieve_for_proposal":ToolAgent("retrieve_for_proposal",
                        tool_func=retriever_tool_func_p,
                        func_desc=retriever_tool_dict_p
                        ),
            "retrieve_for_final_paper":ToolAgent("retrieve_for_final_paper",
                        tool_func=retriever_tool_func,
                        func_desc=retriever_tool_dict
                        )}
