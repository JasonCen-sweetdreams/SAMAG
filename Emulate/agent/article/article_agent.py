# from Emulate.manager import ForumManager
# from Emulate.involvers import System,Tool,search_forum_topk
from Emulate.message import Message
from Emulate.prompt.article import article_prompt_registry

from Emulate.output_parser import article_output_parser_registry

# from langchain.agents import AgentExecutor
from typing import Any, List, Optional, Tuple, Union,Dict,Sequence

from Emulate.memory import BaseMemory,article_memory_registry

from Emulate.memory.article import SocialMemory,RationalMemory
import random
import copy
from agentscope.message import Msg,PlaceholderMessage
from .. import agent_registry

from Emulate.agent.base_agent import BaseGraphAgent

import json
from Emulate.prompt import MODEL
from Emulate.utils.count import count_prompt_len
...
"""
    ...
    def proposal_initialization(self,
                                ...
                                searched_keywords:list = []) -> Msg:
        self.reset_state(mode="proposal_initialization")
        # 构造 prompt
        past_context = self.social_memory.retrieve_recent_chat("all", upper_token=500)
...
"""
        prompt_inputs = {
            "role_description": role_description,
            "topic": research_content.get("topic", ""),
            "past_context": past_context,
            "interested_topics": interested_topics,
            "searched_info":""
        }
        # response = self.step(prompt_inputs)
        # values = response.content.get("return_values", {})
    ...
    def suggestion_generation(self,
                              ...
                              research_content: dict,
                              searched_keywords:list = []) -> Msg:
        self.reset_state(mode="suggestion_generation")
        past_context = self.social_memory.retrieve_recent_chat("all", upper_token=500)
        search_info_template = """
You have already searched the following keywords, so avoid searching them again! 
{searched_info}
...
"""
    ...
    def proposal_generation(self,
        ...
        self.reset_state(mode="proposal_generation")
        past_context = self.social_memory.retrieve_recent_chat("all", upper_token=500)
        prompt_inputs = {
            "role_description": role_description,
            "proposals": json.dumps(proposals),
...
"""
        ...
        final_question    = research_content.get("final_question", research_content.get("topic", ""))
        final_objectives  = research_content.get("final_objectives", [])
        final_method      = research_content.get("final_method", "")
        final_hypotheses  = research_content.get("final_hypotheses", [])
        prompt_inputs = {
        "role_description":role_description,
        # "researcher":researcher_infos,
        "past_context": past_context,
