...
import os
import random
from Emulate.wrapper.agent_group import GroupDiscussAgent
...
class ArticleEnvironment(BaseEnvironment):
    ...
    def __init__(self, 
        ...
        self.ablation_no_collaboration = article_write_configs.pop("ablation_no_collaboration", False)
    ...
    def communication(self):
        ...
        for group_id, research_msg in enumerate(research_msgs):
            ...
            print(f"group {group_id} communication finish! Content: {self.agent_groups[group_id]['content']}")
    ...
    def write(self):
        ...
        for group_id, group_info in enumerate(self.agent_groups):
            group = group_info["authors"]
            assert isinstance(group,GroupDiscussAgent) or isinstance(group, RpcAgent)
            if not group_info["content"]:
                research_msgs.append(None)
                continue
            research_content_msg = group(Msg(
    ...
    def step(self):       
        ...
        if article_write_configs["author_num"] >1 and not self.ablation_no_collaboration:
            self.communication()
        else:
